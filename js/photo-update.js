(function(){
  ...
  document.addEventListener('click',e=>{
    const row=e.target.closest('#assetTable tr[data-id],#recentAssets tr[data-id]');
    if(row&&!e.target.closest('button,input,select,a')) currentAssetId=row.dataset.id||currentAssetId;
  },true);
  ...
  function ensureDialog(){
    ...
    d.innerHTML=`<form method="dialog" class="dialog-card" id="employeePhotoForm">
      ...
      <label>写真① 現物外観<input id="employeePhotoFile1" type="file" accept="image/*" capture="environment"><small style="font-weight:500;color:var(--muted)">未選択なら現在の写真を残します（10MBまで）</small></label>
      <label>写真② ラベル・型番<input id="employeePhotoFile2" type="file" accept="image/*" capture="environment"><small style="font-weight:500;color:var(--muted)">メーカーラベル・型番・製造番号が読める写真（10MBまで）</small></label>
  ...
  async function openPhotoDialog(){
    if(!currentAssetId){ alert('備品を特定できませんでした。備品一覧からもう一度開いてください。'); return; }
    const db=getClient();
    if(!db){ alert('接続準備ができていません。ページを再読み込みしてください。'); return; }
    ...
    d.dataset.assetId=data.id;
    d.dataset.photoPath=data.photo_path||'';
    d.dataset.labelPhotoPath=data.label_photo_path||'';
    d.querySelector('#employeePhotoTitle').textContent=`${data.asset_no||''} / ${data.name||''}`;
    d.querySelector('#employeePhotoFile1').value='';
    d.querySelector('#employeePhotoFile2').value='';
    d.querySelector('#employeePhotoError').textContent='';
  ...
  async function savePhotos(e){
    e.preventDefault();
    const d=document.getElementById('employeePhotoDialog');
    const err=d.querySelector('#employeePhotoError');
    const save=d.querySelector('#employeePhotoSave');
    err.textContent=''; save.disabled=true; save.textContent='保存中…';
    const db=getClient();
    try{
      const assetId=d.dataset.assetId;
      const old1=d.dataset.photoPath||null,old2=d.dataset.labelPhotoPath||null;
      const f1=d.querySelector('#employeePhotoFile1').files?.[0]||null;
      const f2=d.querySelector('#employeePhotoFile2').files?.[0]||null;
      if(!f1&&!f2) throw new Error('更新する写真を1枚以上選んでください。');
      const new1=f1?await upload(db,assetId,f1,'outside'):old1;
      const new2=f2?await upload(db,assetId,f2,'label'):old2;
      const {error}=await db.rpc('update_asset_photos',{p_asset_id:assetId,p_photo_path:new1,p_label_photo_path:new2});
      if(error) throw error;
      ...
      console.error(ex); err.textContent=ex.message||'写真の保存に失敗しました。';
    }finally{
      save.disabled=false; save.textContent='写真を保存';
    }
  }
  ...
  function addPhotoButton(){
    ...
    const actions=detail.querySelector('#assetDetail .dialog-actions');
    if(!actions||document.getElementById('employeePhotoUpdateBtn')) return;
    const btn=document.createElement('button');
    btn.type='button';
    btn.id='employeePhotoUpdateBtn';
    btn.className='secondary';
    btn.textContent='写真更新';
    btn.style.minHeight='48px';
    btn.addEventListener('click',openPhotoDialog);
  ...
  function schedule(){
    if(scheduled) return;
    scheduled=true;
    requestAnimationFrame(()=>{scheduled=false;addPhotoButton();});
  }
  const obs=new MutationObserver(schedule);
  obs.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['open']});
