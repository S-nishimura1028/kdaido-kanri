(function(){
  ...
  style.textContent=`
    ...
    @media(max-width:560px){#publicAssetView .public-grid{grid-template-columns:1fr}#publicAssetView{padding:10px 8px 28px}}
  `;
  ...
  async function load(){
    try{
      ...
      view.querySelector('.public-card').innerHTML=`<div class="asset-title">${esc(a.name)}</div><div class="asset-no">管理No. ${esc(a.asset_no)}</div><div class="public-grid">${rows.map(r=>`<div class="item"><small>${esc(r[0])}</small><div>${esc(r[1])}</div></div>`).join('')}</div><div class="notice">この画面はQRコードから確認できる閲覧専用画面です。購入金額や管理メモなど、管理者向け情報は表示しません。</div>`;
    }catch(e){
      console.error(e);
